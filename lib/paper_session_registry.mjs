...
const ALPHA_DIAGNOSTIC_KEYS = [
  "source_removal_passed",
  "perturbations_passed",
  "conservative_ev_passed",
  "probability_passed",
];
...
function validateSourceRemovalSummary(summary) {
  exactKeys(summary, SOURCE_SUMMARY_KEYS, "certificate source-removal summary");
  if (!new Set(["bullish", "bearish"]).has(summary.base_direction) || typeof summary.passed !== "boolean") {
    throw new Error("certificate source-removal summary is not a directional diagnostic");
  }
  ...
  for (const variant of summary.variants) {
    ...
    if (!new Set(["bullish", "bearish", "neutral"]).has(variant.direction)) throw new Error("certificate source-removal direction is invalid");
    ...
    if (typeof variant.stable_direction !== "boolean" || typeof variant.trade_gate.ok !== "boolean"
      || typeof variant.action_stable !== "boolean" || typeof variant.fixed_candidate_passes !== "boolean") {
      throw new Error("certificate source-removal variant contains invalid diagnostic flags");
    }
    if (!new Set([...ENTRY_ACTIONS, "NO_TRADE"]).has(variant.compiled_action)) {
      throw new Error("certificate source-removal compiled action is invalid");
    }
    if (variant.compiled_candidate_id !== null) assertHash(variant.compiled_candidate_id, "certificate source-removal candidate ID");
    if (variant.compiled_candidate_ev !== null) assertFinite(variant.compiled_candidate_ev, "certificate source-removal compiled EV");
...
function validateCertificate(certificate, certificateSigningSecret) {
  ...
  if (certificate.schema_version !== "risk_certificate.v3"
  ...
  exactKeys(certificate.alpha_confidence_diagnostics, ALPHA_DIAGNOSTIC_KEYS, "certificate alpha diagnostics");
  if (!ALPHA_DIAGNOSTIC_KEYS.every((key) => typeof certificate.alpha_confidence_diagnostics[key] === "boolean")) {
    throw new Error("certificate alpha diagnostics contain an invalid value");
  }
  ...
  validateSourceRemovalSummary(certificate.source_removal_summary);
