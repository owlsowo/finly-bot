...
| Use Alpaca MCP or Alpaca CLI | The official `alpaca-mcp-server==2.2.1` powers the coordinated paper runner. The account produced 15 ETF fill events and a read-only same-clock score through September 2, published in [`competition_forward_profit_2026_09_02.json`](../public/data/competition_forward_profit_2026_09_02.json). | No live options order or fill is claimed. |
| Incorporate options | The competition strategy includes a capped-risk SPY options sleeve. Live v2 lets fresh market momentum compile into a one-contract bull-call or bear-put debit vertical while option skew and model-read news may only confirm, reduce, or stop it. The implementation validates OCC symbols, payoff bounds, quote freshness, liquidity, aggregate risk, and Alpaca multi-leg payload shape. On September 2 it completed 24 live evaluation cycles; all ended `NO_TRADE`, as recorded in [`options_live_decision_funnel_2026_09_02.json`](../public/data/options_live_decision_funnel_2026_09_02.json). | The historical return studies measure the coordinated four-fund sleeve, not historical options P&L. Public verticals are compiler fixtures unless a broker receipt says otherwise. |
...
The event judges paper-account **P&L and trading activity**. Through the September 2 close, Finly's broker equity was **$100,141.24** while a same-$100,000 SPY raw-price benchmark was **$99,715.24** at exactly **4:00 p.m. ET**. That is a **$426.00 advantage**, based on 15 ETF fill events and zero external cashflows. The allocation and result remain frozen. The options sleeve is versioned separately in [`options-policy-revision-v3-2026-09-02.json`](../config/options-policy-revision-v3-2026-09-02.json); earlier abstentions and P&L remain attributed to the policy revision that produced them. The account still has no verified live options fill, so do not force or imply one.
...
1. “Through the market close on 2 September 2026, Finly was up $141.24 while the same-$100,000 SPY raw-price benchmark was down $284.76 at exactly 4:00 p.m. ET, a $426.00 advantage after 15 ETF fill events and with zero external cashflows.” Source: [`competition_forward_profit_2026_09_02.json`](../public/data/competition_forward_profit_2026_09_02.json).
...
5. “During the September 2 session, the options sleeve completed 24 live evaluation cycles: 14 failed certification, six lacked model evidence, and four arrived after the entry cutoff. All 24 ended `NO_TRADE`, with zero options orders, zero options fills, and $0 new options risk.” Source: [`options_live_decision_funnel_2026_09_02.json`](../public/data/options_live_decision_funnel_2026_09_02.json).
6. Attempts 115 and 116 are separate pre-registered research protocols. Their earlier zero-outcome status is not the competition account's status and must not be used to describe the now-measured live account.

Finly must not claim future profitability, next-month SPY outperformance, verified live options P&L, or superiority over a named project. The September 2 score is broker-observed paper performance; it is not the final competition result or a promise about later sessions.
...
| --- | --- | --- |
| **P&L Performance** | September 2 broker observation: +$141.24 for Finly versus -$284.76 for same-clock SPY, a +$426.00 advantage; 15 ETF fill events; zero external cashflows. Historical replays provide a separate longer-horizon rationale. | The live sample is short, and there is no live options P&L yet. |
| **Technology Implementation** | Controlled delegation, signed bullish/bearish options compilation, exact payoff checks, one-contract/$500 execution caps, mutation guards, risk reservations, bounded cancel/reprice exits, idempotency, reconciliation logic, 15 ETF fill events, 24 live options evaluations, read-only same-clock scoring, and 827 automated tests. | The live options sleeve has not yet produced an order/fill/read-back artifact. |
...
1. Run `npm run verify` and reproduce it from a clean clone using the pinned runtime; current suite: 827 total, 825 passed, 0 failed, 2 skipped.
