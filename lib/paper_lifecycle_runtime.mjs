...
export const MAX_EXIT_CANCEL_ATTEMPTS = 2;
...
export class AlpacaPaperLifecycleRuntime {
  ...
  async cancelWorkingExit({ requestId }) {
    ...
    const isRecordedRetry = state.cancel_operation?.request_id === requestId;
    if (!isRecordedRetry && this.exitCancelAttemptCount() >= MAX_EXIT_CANCEL_ATTEMPTS) {
      throw new Error("bounded exit cancel attempts are exhausted");
    }
  ...
  exitCancelAttemptCount() {
    return this.lifecycle.snapshot().history.filter((event) => event.event === "EXIT_CANCEL_REQUESTED").length;
  }
}
