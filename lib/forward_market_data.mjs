...
function makeProvenance({ origin, path, pageCount, request }) {
  return deepFreeze({
    provider: "Alpaca",
    origin,
    path,
...
export class ForwardMarketDataClient {
  ...
  async getMarketCalendar({ start, end, credentials } = {}) {
    ...
    const calendar = requireArray(raw, "market-calendar response").map((entry, index) => {
  ...
  async getCorporateActionAnnouncements(symbol, { start, end, credentials } = {}) {
    ...
    const announcements = requireArray(raw, "corporate-action response").map((action, index) => normalizeAnnouncement(action, {
      index,
      symbol,
      start,
      end,
  ...
  async #readAllPages({ origin, path, query, credentials, label }) {
    ...
    let pageToken;
    while (true) {
      const page = requireRecord(await this.#getJson({
        origin,
        path,
