...
export function createAlpacaPaperPreflight(client, {
  ...
  return async ({ candidate, now = new Date() }) => {
    ...
    const [account, configuration, clock, positions, openOrders, contractPage, latestOptions, latestStock] = await Promise.all([
      client.getAccount(),
    ...
    return {
      ...
      positions: requireArray(positions, "positions"),
      open_orders: requireArray(openOrders, "open orders"),
      open_defined_risk: 0,
