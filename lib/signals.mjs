...
function validateEconomicDirectionAuthority(authority) {
  ...
  if (!new Set([
    "finly_economic_options_direction_authority.v1",
    "finly_economic_options_direction_authority.v2",
  ]).has(authority.schema_version) || authoritySha256 !== sha256(body)) {
  ...
  const enabledV1 = authority.schema_version === "finly_economic_options_direction_authority.v1"
    && authority.decision === "ALLOW_BULLISH_DIRECTION_ONLY"
    && authority.direction === "bullish"
    && authority.maximum_direction_score > 0;
  const enabledV2 = authority.schema_version === "finly_economic_options_direction_authority.v2"
    && authority.decision === "ALLOW_BOUNDED_DIRECTION"
    && authority.direction === "bidirectional"
    && authority.maximum_direction_score > 0;
  const disabled = authority.decision === "NO_TRADE"
    && authority.direction === "neutral"
    && authority.maximum_direction_score === 0;
  if (!enabledV1 && !enabledV2 && !disabled) throw new TypeError("economic direction authority state is invalid");
  return authority;
}

function aggregateUnderEconomicAuthorityV1(rows, valid, authority, { underlying, horizonSessions }) {
  ...
  const supportiveWeight = deterministicRows.reduce(
    (sum, row) => sum + (row.direction > 0 ? row.weight * Math.abs(row.direction) : 0), 0,
  );
  ...
  const modelRiskPenalty = rows.filter((row) => row.family === "events" && row.direction < 0).reduce(
    (sum, row) => sum + (-row.direction) * row.weight / FAMILY_BASE_WEIGHTS.events, 0,
  );
  ...
  return validateIntent({
    schema_version: "finly_intent.v1", underlying, direction,
    ...
    active_weight: round(activeWeight, 6), horizon_sessions: horizonSessions,
...
function aggregateUnderEconomicAuthorityV2(rows, valid, authority, {
  underlying,
  horizonSessions,
  alphaPolicy,
}) {
  ...
  const deterministicRows = rows.filter((row) => row.family !== "events");
  const marketRow = deterministicRows.find((row) => row.family === "market");
  const deterministicWeight = deterministicRows.reduce((sum, row) => sum + row.weight, 0);
  const deterministicVolatility = deterministicWeight === 0 ? 0 : deterministicRows.reduce(
  ...
  const directionalWeight = deterministicRows.reduce((sum, row) => sum + row.weight * Math.abs(row.direction), 0);
  // Raw put/call skew is not permitted to choose the sign. Until a trailing
  // DTE/moneyness-normalized options baseline exists, price momentum owns the
  // sign and options/events can only confirm or reduce that bounded view.
  const deterministicSign = Math.sign(marketRow?.direction ?? 0);
  const supportiveWeight = deterministicRows.reduce(
    (sum, row) => sum + (Math.sign(row.direction) === deterministicSign ? row.weight * Math.abs(row.direction) : 0),
    0,
  );
  const deterministicAgreement = directionalWeight === 0 ? 0 : supportiveWeight / directionalWeight;
  const opposingEvidencePenalty = rows.filter(
    (row) => new Set(["options", "events"]).has(row.family)
      && deterministicSign !== 0
      && Math.sign(row.direction) === -deterministicSign,
  ).reduce(
    (sum, row) => sum + Math.abs(row.direction) * row.weight / FAMILY_BASE_WEIGHTS[row.family],
    0,
  );
  const modelRiskMultiplier = clip(1 - opposingEvidencePenalty, 0, 1);
  const supportedDirectionScore = deterministicSign === 0
    ? 0
    : deterministicSign * Math.min(authority.maximum_direction_score, Math.abs(marketRow.direction)) * modelRiskMultiplier;
  const direction = Math.abs(supportedDirectionScore) < alphaPolicy.directionThreshold
    ? "neutral"
    : supportedDirectionScore > 0 ? "bullish" : "bearish";
  const activeWeight = Math.min(1, authority.authority_weight + deterministicWeight);
  return validateIntent({
...
export function aggregateSignals(signals, {
  ...
  if (authority) {
    const aggregate = authority.schema_version.endsWith(".v1")
      ? aggregateUnderEconomicAuthorityV1
      : aggregateUnderEconomicAuthorityV2;
    return aggregate(rows, valid, authority, { underlying, horizonSessions, alphaPolicy });
  }
...
export function intentCanTrade(intent, alphaPolicy = POLICY) {
  validateIntent(intent);
  if (intent.direction === "neutral") return { ok: false, reason: "DIRECTION_BELOW_THRESHOLD" };
  if (intent.coverage < alphaPolicy.minCoverage) return { ok: false, reason: "INSUFFICIENT_COVERAGE" };
  if (intent.agreement < alphaPolicy.minAgreement) return { ok: false, reason: "INSUFFICIENT_AGREEMENT" };
  return { ok: true };
}
...
export function leaveOneFamilyOut(signals, options) {
  ...
  const variants = families.map((removed_family) => {
    ...
    return {
      ...
      // Removal may reduce a view to neutral, but it must never produce the
      // opposite trade. Candidate/action equality remains a downstream
      // diagnostic rather than a duplicate authorization requirement.
      stable_direction: options?.allowNeutralSourceRemoval
        ? intent.direction === "neutral" || intent.direction === base.direction
        : intent.direction === base.direction && intent.direction !== "neutral",
      trade_gate: intentCanTrade(intent, options?.alphaPolicy ?? POLICY),
    };
  });
  return {
