...
export function enumerateVerticals(intent, optionChain) {
  validateIntent(intent);
  const tradeGate = intentCanTrade(intent);
  ...
  for (let left = 0; left < valid.length; left += 1) {
    for (let right = left + 1; right < valid.length; right += 1) {
      ...
      if (rewardRisk < POLICY.minimumRewardRisk) {
...
export function evaluateCandidate(candidate, intent, market) {
  ...
  const requiredEv = Math.max(POLICY.minimumEvDollars, POLICY.minimumEvToMaxLoss * candidate.max_loss);
  return {
    ...candidate,
    ...
    passes_probability: probabilityProfit >= POLICY.minimumProbabilityOfProfit,
  };
}

export function compileIntent(intent, optionChain, market) {
  const enumeration = enumerateVerticals(intent, optionChain);
  const evaluated = enumeration.accepted.map((candidate) => evaluateCandidate(candidate, intent, market));
  const eligible = evaluated
    .filter((candidate) => candidate.passes_ev && candidate.passes_probability && candidate.max_loss <= POLICY.riskPerTradeDollarCap)
    .sort((left, right) => {
