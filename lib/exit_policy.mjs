...
export function evaluateDebitSpreadExit({
  ...
  const executableCredit = Math.max(0.01, Math.floor(Math.max(0, rawCredit) * 100) / 100).toFixed(2);
  ...
  else if (forceExitInstant !== null && observationInstant >= forceExitInstant) trigger = "competition_end_guard";
  ...
  return {
    ...
    executable_credit_limit: executableCredit,
    credit_limit: trigger ? executableCredit : null,
