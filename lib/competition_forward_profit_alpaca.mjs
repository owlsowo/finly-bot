...
function latestCompletedRegularWindowStart(contract, maximumValuedAt, sessions) {
  const competitionStartMs = Date.parse(contract.competition_window.start_at);
  const competitionEndMs = Date.parse(contract.competition_window.end_at);
  const maximumValuedAtMs = Date.parse(maximumValuedAt);
  let latestWindowStartMs = null;
  for (const session of sessions.values()) {
    const sessionOpenMs = Date.parse(session.open_at);
    const sessionCloseMs = Date.parse(session.close_at);
    const valuedAtMs = Math.min(maximumValuedAtMs, sessionCloseMs, competitionEndMs - WINDOW_DURATION_MS);
    const windowStartMs = valuedAtMs - WINDOW_DURATION_MS;
    if (windowStartMs < sessionOpenMs || windowStartMs < competitionStartMs) continue;
    if (latestWindowStartMs === null || windowStartMs > latestWindowStartMs) {
      latestWindowStartMs = windowStartMs;
    }
  }
  return latestWindowStartMs === null ? null : new Date(latestWindowStartMs).toISOString();
}

export function buildAlpacaForwardProfitRequestPlan(contract, pollStartedAt, marketCalendar = null) {
  assertAlpacaActivityMappingMatchesContract(contract);
  ...
  const lastWindowStartAt = marketCalendar === null
    ? bounds.lastWindowStartAt
    : latestCompletedRegularWindowStart(
      contract, bounds.commonValuedAt, normalizeCalendar(marketCalendar),
    );
  if (lastWindowStartAt === null) return null;
  return Object.freeze({
    ...
    activities: Object.freeze({
      path: "/v2/account/activities",
      query: Object.freeze({
        after: contract.competition_window.start_at,
        until: bounds.activityUntilExclusive,
        direction: "asc",
        page_size: String(ACTIVITY_PAGE_SIZE),
      }),
...
export class AlpacaForwardProfitReadClient {
  ...
  async measure({ contract, activityBaseline }) {
    ...
    const preliminaryPlan = buildAlpacaForwardProfitRequestPlan(contract, pollStartedAt);
    if (preliminaryPlan === null) return emptyMeasurement(contract, pollStartedAt);
    const preWindowOrders = await this.#get(
      this.#tradingBase,
      preliminaryPlan.pre_window_order_guard.path,
      preliminaryPlan.pre_window_order_guard.query,
    );
    assertAlpacaPreWindowOrderGuard(preWindowOrders, activityBaseline, contract);
    const marketCalendar = await this.#get(
      this.#tradingBase, preliminaryPlan.calendar.path, preliminaryPlan.calendar.query,
    );
    const plan = buildAlpacaForwardProfitRequestPlan(contract, pollStartedAt, marketCalendar);
