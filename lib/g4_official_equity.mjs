...
function assertState(state, protocol) {
  ...
  if (state.protocol_hash !== protocol.protocol_hash
    ...
    || Math.abs(Number(state.baseline_equity) - 100_000) > BASELINE_TOLERANCE_DOLLARS
...
function brokerOrderResult(order, projection) {
  ...
  if (status === "partially_filled") return { kind: "FAILED", reason: "PARTIAL_FILL" };
...
export async function runG4OfficialEquityCycle({
  ...
  if (state?.phase === "READY") {
    try {
      const broker = await readBroker(client);
  ...
  if (nextLegIndex === -1) {
    ...
    const held = new Set(view.equityPositions.map((position) => position.symbol));
    if (view.equityOrders.length !== 0 || held.size !== 4 || G4_EQUITY_SYMBOLS.some((symbol) => !held.has(symbol))
      || totalFilled < authorizedTotal - 20
    ...
    try { assertReadyEquityQuantities(view, state); }
      ...
      state = frozenState(state, observedAt);
      await saveState(store, signingSecret, protocol, state, state.revision - 1);
      return publicResult("G4_EQUITY_FROZEN", state, protocol, { reason: "FINAL_QUANTITY_RECONCILIATION_FAILED" });
    }
