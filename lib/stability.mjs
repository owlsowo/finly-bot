...
export function perturbationReport(signals, optionChain, fixedCandidate, market, options = {}) {
  const alphaPolicy = options.alphaPolicy ?? POLICY;
  const liveDiagnostics = options.liveAlphaDiagnostics === true;
  ...
  for (let index = 1; index <= POLICY.perturbationCount; index += 1) {
    ...
    const evaluated = evaluateCandidate(perturbedCandidate, intent, perturbedMarket, { alphaPolicy });
    const recompiled = compileIntent(intent, perturbedChain, perturbedMarket, options);
    rows.push({
      run: index,
  ...
  const directionFlips = rows.filter((row) => liveDiagnostics
    ? row.direction !== "neutral" && row.direction !== baseIntent.direction
    : row.direction !== baseIntent.direction).length;
  return {
    count: rows.length,
    ...
    passed: liveDiagnostics
      ? directionFlips <= alphaPolicy.perturbationMaximumDirectionFlips
        && nonzeroDirectionRate >= alphaPolicy.perturbationMinimumNonzeroDirectionRate
        && tradeRate >= alphaPolicy.perturbationMinimumTradeRate
        && sameStructureRate >= alphaPolicy.perturbationMinimumSameStructureRate
        && p5 >= 0
      : directionFlips === 0
        && nonzeroDirectionRate >= 0.90
        && tradeRate >= 0.80
        && sameStructureRate >= 0.75
        && p5 > 0,
    rows,
  };
}
...
export function runStabilityGate(signals, optionChain, market, options = {}) {
  ...
  const liveDiagnostics = options.liveAlphaDiagnostics === true;
  const baseIntent = aggregateSignals(signals, options);
  const compilation = compileIntent(baseIntent, optionChain, market, options);
  ...
  const sourceRows = sourceRemoval.variants.map((variant) => {
    ...
    const variantCompilation = compileIntent(variantIntent, optionChain, market, options);
    const fixedCandidate = evaluateCandidate(compilation.selected, variantIntent, market, { alphaPolicy });
    return {
      ...variant,
  ...
  const sourcePassCount = sourceRows.filter((row) => row.stable_direction).length;
  const sourcePassed = liveDiagnostics
    ? sourceRows.length > 0
      && sourcePassCount / sourceRows.length >= alphaPolicy.sourceRemovalMinimumPassRate
    : sourceRows.every((row) => row.stable_direction
      && row.trade_gate.ok
      && row.action_stable
      && row.fixed_candidate_passes);
