...
export const CALIBRATION_SCHEMA = "finly_options_policy_calibration.v1";
const SPOT = 560;
...
export function buildOptionsPolicyCalibration(input) {
  ...
  for (let index = LOOKBACK_RETURNS; index < input.rows.length; index += STRIDE_SESSIONS) {
    ...
    sampleCount += 1;
    if (fair.selected) fairEligible += 1;
  ...
  const body = {
    ...
    results: {
      fair_surface: {
        eligible_count: fairEligible,
        eligible_rate: round(fairEligible / sampleCount, 8),
      },
      favorable_surface: {
        eligible_count: improvedEligibleWindows.length,
        eligible_rate: round(improvedEligibleWindows.length / sampleCount, 8),
        eligible_windows: improvedEligibleWindows,
      },
    },
  };
