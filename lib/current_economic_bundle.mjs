...
export function currentPaperAllocation(account, positions) {
  ...
  const unsupported = positions.filter((position) => !new Set(["SPY", "BIL"]).has(position?.symbol));
  ...
  const spyMarketValue = positions.filter((position) => position.symbol === "SPY")
    .reduce((sum, position) => sum + finiteNonnegative(position.market_value, "SPY market value"), 0);
  if (spyMarketValue > equity * 1.000001) throw new Error("SPY market value exceeds paper equity");
  const spyWeight = Math.min(1, spyMarketValue / equity);
