...
// 1 — Product first.
{
  ...
  addKicker(slide, "2013–2026 historical simulation · after modeled costs", C.mint, 64, 126);
  addText(slide, "Finly finished $38,629 ahead of SPY.", 64, 170, 540, 222, { fontSize: 54, bold: true, color: C.white });
  addText(slide, "The same $10,000 became $106,711 with Finly and $68,082 with the S&P 500 tracker. The strategy now runs as a checked Alpaca paper-trading product.", 64, 400, 530, 108, { fontSize: 21, color: "#D7E3E5" });
  ...
  addMetric(slide, "$106,711", "Finly historical ending wealth", 64, 552, 250, { valueSize: 38, valueColor: C.gold, labelColor: "#C5D4D8", labelSize: 12 });
  addMetric(slide, "$68,082", "SPY historical ending wealth", 360, 552, 240, { valueSize: 38, valueColor: C.white, labelColor: "#C5D4D8", labelSize: 12 });
  addMetric(slide, "+$426", "separate live paper advantage", 650, 552, 250, { valueSize: 38, valueColor: C.mint, labelColor: "#C5D4D8", labelSize: 12 });
...
// 9 — Live call to action.
{
  ...
  addText(slide, "See why Finly finished $38,629 ahead of SPY.", 64, 80, 790, 118, { fontSize: 50, bold: true });
  addText(slide, "Explore the 2013–2026 cost-modeled replay, then inspect the separate +$426 Alpaca paper result and every decision behind it.", 64, 208, 780, 76, { fontSize: 26, color: C.gray });
