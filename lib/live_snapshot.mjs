...
function normalizeContract(raw, { underlying, expectedType, minimumExpiry, maximumExpiry, minimumStrike, maximumStrike }) {
  ...
  const openInterest = requireFinite(requireOwn(contract, "open_interest", `option contract ${symbol}`), `option contract ${symbol} open interest`, { minimum: 0, integer: true });
...
function normalizeOptionChain(contracts, snapshots, { underlying, optionFeed, asOf, marketDate }) {
  ...
  for (const contract of contracts.values()) {
    ...
    if (!snapshots.has(contract.symbol)) throw new Error(`option snapshot is missing for ${contract.symbol}`);
