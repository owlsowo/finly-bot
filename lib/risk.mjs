...
export function calculateQuantity(candidate, account, { halfRisk = false } = {}) {
  ...
  return Math.max(0, Math.min(POLICY.maxContracts, Math.floor(riskBudget / candidate.max_loss)));
}
...
export function createRiskCertificate({
  ...
  const halfRisk = stability.perturbations?.rejected_variants > 0;
  const quantity = selected ? calculateQuantity(selected, account, { halfRisk }) : 0;
  ...
  const body = {
    schema_version: "risk_certificate.v2",
    run_id: runId,
